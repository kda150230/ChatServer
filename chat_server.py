        ...
        try:
            ...
            if message:
                if "Chat" in message.decode():
                    chat_rcvd(message.decode()[5:-1], conn)#send requested client and socket
...
def chat_rcvd(mess, connection):#message
    print ("Chat Request Received")
    print ("Chat Requested with " + mess)#
    for IDs in clientIDs:
        if IDs == mess:
            print("Client Found")
            for clients in list_of_clients: #needs to only send message to specified client, not all
                print (str(clients))
                if clients!=connection:
                    try:
                        clients.send("Chat Started".encode())
                    except:
                        clients.close()
                        remove(clients)
